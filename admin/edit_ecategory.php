<?php
    ...
    $cat_id="";
    $cat_name="";
    $query="select *from e_category where cat_id='$_GET[cat_id]'";
...
<head>
    ...
    .custom-btn:hover {
        background-color: #3a6ea5; /* Darker Blue on Hover */
        ...
        <div class="col-md-4">
            <form action="" method="post" onsubmit="return validateFunction();">
                <div class="form-group mb-3">
                    ...
                    <input type="text" name="book_name" value="<?php echo $cat_name;?>" class="form-control" required="" id="cat_name">
                </div>
...
if (isset($_POST['update']) && isset($_GET['cat_id'])) {
    $cat_id = mysqli_real_escape_string($connection, $_GET['cat_id']);
    $cat_name = mysqli_real_escape_string($connection, $_POST['book_name']); 

    // Update query
    $query = "UPDATE e_category SET cat_name = ? WHERE cat_id = ?";
    ...
    if ($stmt) {
        ...
        if ($execute) {
            echo "<script>alert('Category updated successfully'); window.location.href='manage_ecategory.php';</script>";
        } else {
