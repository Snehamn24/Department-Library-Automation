<?php
    // Start session and database connection
    require('functions.php');
    session_start();
    $connection = mysqli_connect("localhost", "root", "");
    $db = mysqli_select_db($connection, "lms");

    // Initialize variables for book details
    $book_num = $professor_id = $professor_name = $book_name = $book_image  = "";

    //fetching the values
    if (isset($_GET['professor_id']) && isset($_GET['book_num'])) {
    $professor_id = $_GET['professor_id'];
    $book_num = $_GET['book_num'];


        // Fetch book details from the database
        $query = "SELECT 
            p.professor_id,
            p.professor_name,
            p.book_num,
            p.book_name,
            d.book_image,
            d.no_copy
        FROM 
            pro_book_requests p
        JOIN 
            dept_books d ON p.book_num = d.book_no
        WHERE 
            p.professor_id = '$professor_id' AND p.book_num = '$book_num'";

        $query_run = mysqli_query($connection, $query);

        // Fetch the first row
        if ($row = mysqli_fetch_assoc($query_run)) {
            $professor_id = $row['professor_id'];
            $professor_name = $row['professor_name'];
            $book_num = $row['book_num'];
            $book_name = $row['book_name'];
            $book_image = $row['book_image'];
            $no_of_copy = $row['no_copy'];
        }
    }

    // When the form is submitted
    if (isset($_POST['issue'])) {
        // Get form data
        $professor_id = $_POST['p_id'];
        $professor_name = $_POST['p_name'];
        $book_num = $_POST['book_num']; 
        $book_name = mysqli_real_escape_string($connection, $_POST['book_name']);
        $return_date = $_POST['return_date'];
        $book_image = mysqli_real_escape_string($connection, $_POST['book_image']);

        // Full image path
        $image_url = "image/" . $book_image;

        // Check if the same professor has already been issued the same book
        $check_query = "SELECT * FROM pro_issue_book WHERE p_id = '$professor_id' AND book_num = '$book_num' AND status = 'Issued'";
        $check_result = mysqli_query($connection, $check_query);

        if (mysqli_num_rows($check_result) > 0) {
            echo "<script>alert('This professor has already been issued this book!'); window.location.href='issue_probook.php';</script>";
        } else {
            // Insert query to issue the book
            $query = "INSERT INTO pro_issue_book (p_id, p_name, book_num, book_image, book_name, return_date, status)
                      VALUES ('$professor_id', '$professor_name', '$book_num', '$image_url', '$book_name', '$return_date', 'Issued')";
            $action_update = "UPDATE pro_book_requests SET status = 'Approved' WHERE professor_id = '$professor_id' AND book_num = '$book_num'";
            mysqli_query($connection, $action_update);

            // Execute query
            if (mysqli_query($connection, $query)) {
                // Reduce the no_copy count by 1
                $update_query = "UPDATE dept_books SET no_copy = no_copy - 1 WHERE book_no = '$book_num'";
                if (mysqli_query($connection, $update_query)) {
                    echo "<script>alert('Book issued successfully and stock updated!'); window.location.href='issue_probook.php';</script>";
                } else {
                    echo "<script>alert('Book issued but stock update failed: " . mysqli_error($connection) . "');</script>";
                }
            } else {
                echo "<script>alert('Error issuing the book: " . mysqli_error($connection) . "');</script>";
            }
        }
    }
?>

<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" name="viewport" content="width=device-width, initial-scale=1">
    <title>Library Management System (LMS)</title>
    <link rel="stylesheet" type="text/css" href="bootstrap-5.3.3-dist/css/bootstrap.min.css">
    <script type="text/javascript" src="bootstrap-5.3.3-dist/js/jquery_latest.js"></script>
    <script type="text/javascript" src="bootstrap-5.3.3-dist/js/bootstrap.bundle.min.js"></script>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons/font/bootstrap-icons.css">
    <link rel="stylesheet" type="text/css" href="admin_style.css">
    <style type="text/css">
         body {
            background: #d6e6f8;
        }
        .btn-primary
        {
           background: #3a6ea5;
           border: none;
           border-radius: 30px;
           padding: 12px 25px;
           font-size: 1.1em;
           margin: 10px 0;
           width: 50%;
          color: white;
          transition: 0.4s;
        }
        .btn-primary:hover {
      background: #3a6ea5;
      transform: scale(1.05);
      color: #f9f9f9;
  }
    }
      table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 20px;
            table-layout: fixed;
        }
        th, td {
            border: 1px solid black;
            padding: 10px;
            text-align: center;
            word-wrap: break-word;
        }
        th {
            background-color: #f2f2f2;
        }
        img {
            width: 100px; /* Adjust as needed */
            height: auto;
        }
    </style> 
</head>
<body>
     <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <div class="container-fluid">

            <div class="topbar">
   
   <i class="bi bi-list menu-icon" id="toggleBtn" style="font-size: 1.2rem; margin-right: 1px; color: white;margin-left: 1px;"></i>
&nbsp;&nbsp; 
  </div><div class="navbar-header">
                 <img src="../booklogo.png" alt="booklogo" class="img-fluid" style="height: 50px; width: 50px; padding-bottom: 5px;"> 
        &nbsp;
                <a class="navbar-brand" href="admin_dashboard.php">Dashboard</a>
            </div>&nbsp;&nbsp;&nbsp;
            <font style="color:white"> <span> <strong> Welcome : <?php echo $_SESSION['name'];?> </strong> </span> </font> &nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <font style="color:white"> <span> <strong> Email : <?php echo $_SESSION['email'];?> </strong> </span> </font>
        
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav"
                aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                            data-bs-toggle="dropdown" aria-expanded="false"><img src="../person_icon.png" alt="Profile" style="height: 25px; width: 25px; margin-right: 5px;">
                            My Profile
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <li><a class="dropdown-item" href="view_profile.php"><i class="bi bi-person-circle"></i>
                            View Profile</a></li>
                            <li><a class="dropdown-item" href="edit_profile.php"><i class="bi bi-pen-fill"></i>
                            Edit Profile</a></li>
                            <li><a class="dropdown-item" href="change_password.php"><i class="bi bi-key-fill"></i>
                            Change Password</a></li>
                        </ul>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link d-flex align-items-center" href="../logout.php">
                        <i class="bi bi-box-arrow-right" style="font-size: 1.2rem; margin-right: 5px;"></i> Logout
                    </a>
                    </li>
                </ul>
            </div>
        </div>
    </nav>
    <!-- Sidebar -->
  <div class="sidebar" id="sidebar">
    <ul class="nav flex-column">
      <li class="l1"><a href="admin_dashboard.php">Dashboard</a></li>
      <li>
        <a class="dropdown-toggle" data-bs-toggle="collapse" href="#ebooks">E-Books</a>
        <div class="collapse" id="ebooks">
          <ul class="list-unstyled ps-3">
            <li><a href="add_books.php">Add New E-Book</a></li>
            <li><a href="manage_book.php">Manage E-Books</a></li>
          </ul>
        </div>
      </li>
      <li>
        <a class="dropdown-toggle" data-bs-toggle="collapse" href="#category">Category</a>
        <div class="collapse" id="category">
          <ul class="list-unstyled ps-3">
            <li><a href="add_category.php">Add New Category</a></li>
            <li><a href="manage_category.php">Manage Category</a></li>
          </ul>
        </div>
      </li>
      <li>
        <a class="dropdown-toggle" data-bs-toggle="collapse" href="#deptbooks">Department Books</a>
        <div class="collapse" id="deptbooks">
          <ul class="list-unstyled ps-3">
            <li><a href="add_deptbooks.php">Add Dept. Books</a></li>
            <li><a href="manage_deptbooks.php">Manage Dept. Books</a></li>
          </ul>
        </div>
      </li>
       <li>
        <a class="dropdown-toggle" data-bs-toggle="collapse" href="#fine">Fine Calculation</a>
        <div class="collapse" id="fine">
          <ul class="list-unstyled ps-3">
            <li><a href="admin_professor_fine.php">Professor</a></li>
            <li><a href="admin_student_fine.php">Students</a></li>
          </ul>
        </div>
      </li>
    </ul>
  </div>

    

    <span><marquee>This is library management system. Library opens at 8:00 AM and closes at 8:00 PM</marquee></span><br><br>

    <div class="row">
        <div class="col-md-4"></div>
        <div class="col-md-4">
            <form action="" method="post">
                <div class="form-group mb-3">
                    <label>Professor-Id:</label>
                    <input type="text" name="p_id" value="<?php echo $professor_id; ?>" class="form-control" required>
                </div>
                <div class="form-group mb-3">
                    <label>Professor Name:</label>
                    <input type="text" name="p_name" value="<?php echo $professor_name; ?>" class="form-control" required>
                </div>
                <div class="form-group mb-3">
                    <label>Book Number:</label>
                    <input type="text" name="book_num" value="<?php echo $book_num; ?>" class="form-control" required>
                </div>
                <div class="form-group mb-3">
                    <label>Book Image:</label><br>
                    <img src="image/<?php echo $book_image; ?>" alt="User Image" width="100">
                    <input type="hidden" name="book_image" value="<?php echo $book_image; ?>">
                </div>
                <div class="form-group mb-3">
                    <label>Book Name:</label>
                    <input type="text" name="book_name" value="<?php echo $book_name; ?>" class="form-control" required>
                </div>
                <div class="form-group mb-3">
                    <label>Return Date:</label>
                    <input type="date" name="return_date" value="<?php echo date('Y-m-d', strtotime('+45 days')); ?>" class="form-control" required>
                </div>
                <button class="btn btn-primary" name="issue">Issue Book</button>
            </form>
        </div>
        <div class="col-md-4"></div>
    </div>
<script>
  const toggleBtn = document.getElementById('toggleBtn');
  const sidebar = document.getElementById('sidebar');

  // Toggle sidebar open/close
  toggleBtn.addEventListener('click', function () {
    sidebar.classList.toggle('active');
  });

  // Optional: Close sidebar when clicking outside of it
  document.addEventListener('click', function (event) {
    if (!sidebar.contains(event.target) && !toggleBtn.contains(event.target)) {
      sidebar.classList.remove('active');
    }
  });
</script>
</body>
</html>
